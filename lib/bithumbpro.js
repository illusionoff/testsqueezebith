...
function wsStartBith(cmd, args, initialGate, writableFiles) {
  ...
  function Squeeze(arr) {
    ...
    let arrTemp = arr.map((item) => item[0]);
  ...
  function average(arr, strItem) {
    const sum = arr.reduce((accum, item) => accum += item[strItem]);
    const average = sum / arr.length;
    // const averageRound = average.round(1);//округляем
    const averageRound = Math.round(average);
    console.log('average(arr, strItem)  arr=', arr);

    console.log('average(arr, strItem)  sum=', sum);
    return averageRound
  }
  let timerWritting;
  function startTimerWritting() {
    timerWritting = setInterval(function () {
      ...
      let s = initialBith.arrChart.s;
      let result = Squeeze(b);
      let result2 = Squeeze(s);
      ...
      if (result || result2) {
        let str = 'b,\n';
        const arrBay = b.map((item) => item + '\n'); //"\r\n"
        str += arrBay.join('');//.join()
        str += 's,\n';
        const arrsell = s.map((item) => item + '\n'); //"\r\n"
        str += arrsell.join('');
        console.log('str=', str);

        function computes(arr) {
          const diffTimeVer = average(arr, 4);
          const diffTimeServer = average(arr, 6);
          return { diffTimeVer, diffTimeServer }
        }
        let computesBay = computes(b);
        let computesSell = computes(s);
        // console.log('average(arr[4])=', average(arr));

        // console.log('average(arr[6]=', average(arr[6]));

        console.log('computesBay=', computesBay);
        console.log('computesSell=', computesSell);
        const strComputes = `averag diffTimeVer b = ${computesBay.diffTimeVer} \n diffTimeVer s = ${computesSell.diffTimeVer}
        averag diffTimeServer b = ${computesBay.diffTimeServer} \n averag diffTimeServer s = ${computesSell.diffTimeServer}`
      ...
      b.length = 0;
      s.length = 0;
      console.log('initialBith.arrChart.b=', b);
      console.log('initialBith.arrChart.s=', s);
