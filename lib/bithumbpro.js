...
function wsStartBith(cmd, args, initialGate, writableFiles) {
  ...
  function startTimerWritting() {
    timerWritting = setInterval(function () {
      // objArrs: { arrBay: [], arrSell: [], arrTimeBay: [], arrTimeSell: [] }
      let strArr = initialBith.objArrs.arrBay.map((item) => item + "\r\n");
      strArr = strArr.join();
      const timeNow = new Date().getTime();
      fs.writeFile(`logs/${timeNow}_testQueezeBith.txt`, strArr, function (error) {
    ...
    if (initialBith.messageObj.code === "00007") {
      ...
      if (Number(initialBith.messageObj.data.ver) === initialBith.ver + 1) {
        ...
        if (initialBith.messageObj.data.b.length === 1) {
          orderbookChange(initialBith.allOrderbookBay, initialBith.messageObj.data.b)
          // initialBith.bayOrSell = 1;
          bayOrSell = 1;

        }
        if (initialBith.messageObj.data.s.length === 1) {
          orderbookChange(initialBith.allOrderbookSell, initialBith.messageObj.data.s)
          initialBith.bayOrSell = 0;
          if (bayOrSell === 1) {
            bayOrSell = 2;
          } else {
            bayOrSell = 0;
