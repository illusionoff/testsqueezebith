...
const fs = require("fs");
const { goTrade, reconnectTimeMessageClosure, changeTradeArr, diffMaxIndexS } = require('../functions/functions');
  ...
  function startTimerWritting() {
    timerWritting = setInterval(function () {
      ...
      if (result || result2) {
        ...
        let arrTempSell = b.map((item) => item[1]);
        let arrTempBay = b.map((item) => item[2]);
        console.log('arrTemp.arrTempSell=', arrTempSell.length);
        console.log('arrTempSell=', arrTempSell);
        console.log('arrTempBay=', arrTempBay);
        console.log('diffMaxIndexS({ arr: arrTemp, sell: true })=', diffMaxIndexS({ arr: arrTempSell, sell: true }));
        console.log('diffMaxIndexS({ arr: arrTemp, sell: false })=', diffMaxIndexS({ arr: arrTempBay, sell: false }));
